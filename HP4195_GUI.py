...
class MainWindow():
    ...
    def measurementFunctionSwitch(self, measFunc):
        print self.analyzer.setMeasurementMode(measFnc)
        self.setActiveTabs()
    ...
    def buildSettingsTabNetwork(self):
        ...
        ##### Port Radiobuttons #####
        pw1.add(self.radioFrame(pw1, self.getSortedList(self.analyzer.NetworkModePorts, 'items'), "Port", self.portNetwork, 'top', 0, self.setPort)) 
        self.portNetwork.get()
        ##### Format Radiobuttons ##### 
        pw1.add(self.radioFrame(pw1, self.getSortedList(self.analyzer.NetworkModeFormats, 'items'), "Format", self.formatNetwork, 'top', 1, self.setFormat))
        pw2 = PanedWindow(pwFull,orient=VERTICAL)
        pw2.pack()       
        ##### Angle Mode Radiobuttons #####
        pw2.add(self.radioFrame(pw2, self.getSortedList(self.analyzer.AngleMode, 'items'), "Angle Mode", self.angleMode, 'top', 1, self.setAngle))
    ...
    def buildSettingsTabSpectrum(self):
        ...
        pw1.pack(fill=BOTH,expand=1)
        ##### Port Radiobuttons #####
        pw1.add(self.radioFrame(pw1, self.getSortedList(self.analyzer.SpectrumModePorts, 'items'), "Port", self.portSpectrum, 'top', 1, self.setPort))
        ...
        pw11.add(self.radioFrame(pw11, self.getSortedList(self.analyzer.SpectrumModeTrackingGenOut, 'items'), "Tracking Generator Output", self.trackGenOut, 'top', 0, self.setTrackingGeneratorOutput)) # Default = 0
        pw1.add(pw11)
        pw2 = PanedWindow(pwFull,orient=VERTICAL)
        pw2.pack()
        ##### Angle Mode Radiobuttons #####
    ...
    def buildSettingsTabImpedance(self):
        ...
        pw1.pack(fill=BOTH,expand=1)
        ##### Port Radiobuttons #####
        pw1.add(self.radioFrame(pw1,self.getSortedList(self.analyzer.ImpedanceModePorts, 'items'), "Port", self.portNetwork, 'top', 1, self.setPort))
        ##### Format Combobox #####             
    ...
    def buildSettingsTabS1122(self):
        ...
        pw1.pack(fill=BOTH,expand=1)
        ##### Format Radiobuttons #####
        pw1.add(self.radioFrame(pw1, self.getSortedList(self.analyzer.SModeFormats, 'items'), "Format", self.formatS1122, 'top', 1, self.setFormat))
        pw2 = PanedWindow(pwFull,orient=VERTICAL)
        pw2.pack()
        ##### Angle Mode Radiobuttons #####
        pw2.add(self.radioFrame(pw2, self.getSortedList(self.analyzer.AngleMode, 'items'), "Angle Mode", self.angleMode, 'top', 1, self.setAngle))
    ...
    def buildSettingsTabS2112(self):
        ...
        pw1.add(self.radioFrame(pw1, FMTS,"Format",self.formatS2112,'top',1, self.setFormat))
        pw2 = PanedWindow(pwFull,orient=VERTICAL)
        pw2.pack()
        ##### Angle Mode Radiobuttons #####
        pw2.add(self.radioFrame(pw2, self.getSortedList(self.analyzer.AngleMode, 'items'), "Angle Mode", self.angleMode, 'top', 1, self.setAngle))
    ...
    def setSpectrumFormat(self, event):
        print self.analyzer.setPort(self.unitSpectrum.get())
    ...
    def buildStimulustab(self):
        ...
        pwLeft.add(self.radioFrame(pwLeft, self.getSortedList(self.analyzer.SweepParameters, 'items'), "Sweep Parameter", self.sweepParameter, 'top', 1, self.setSweepParameter))
        ##### Sweep Type Radiobuttion #####
        pwLeft.add(self.radioFrame(pwLeft, self.getSortedList(self.analyzer.SweepModes, 'items'), "Sweep Type", self.sweepType, 'top', 1, self.setSweepType))
        pwTop.add(pwLeft)
        ...
        pwRight.add(self.radioFrame(pwRight, self.getSortedList(self.analyzer.TriggerModes, 'items'), "Trigger Mode", self.triggerMode, 'top', 1, self.setTriggerMode))
...
############################# Labeled Entry Fields ############################  
    ...
    def buildCalibrationtab(self):
        ...
        self.frCalMode = self.radioFrame(superFrCalMode, self.getSortedList(self.analyzer.CalibrationModes, 'items'), "Calibration Mode", self.calMode, 'left', 1, self.setCalibrationMode)
    ...
    def calibSettingsCallback(self):
        ...
        pw.add(self.radioFrame(pw, self.getSortedList(self.analyzer.CharacteristicImpedances, 'items'), "Characteristic Impedance", self.charImp, 'left', 1, self.setCharacteristicImpedance))
        ### Open ###
    ...
    def radioFrame(self, parent, paramList, label, var, orient, dflt, cmd):
        ...
        for txt, val in paramList:
            rb = Radiobutton(lf, text = txt, value = i, variable = var, command = partial(cmd,txt), indicatoron = 1)
